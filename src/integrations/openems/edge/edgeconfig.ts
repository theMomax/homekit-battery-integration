...
export class EdgeConfig {
     ...
     * Get all Channels.
     */
    public getChannels(): Map<String, EdgeConfig.ComponentChannel> {
        let channels = new Map<String, EdgeConfig.ComponentChannel>()
        for (const componentId in this.components) {
            if (!this.components[componentId].isEnabled) {
                continue
            }
            for (const channelId in this.components[componentId].channels) {
                channels.set(componentId + '/' + channelId, this.components[componentId].channels[channelId])
            }
        }
        return channels
    }
